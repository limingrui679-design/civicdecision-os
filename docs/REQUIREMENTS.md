...
| RELEASE-004 | Signed release, checksums and SBOM | in_progress | Local candidate emits a validated CycloneDX 1.6 SBOM, 14-entry portable asset checksum inventory, deterministic release bundle, and detached bundle SHA-256. A cryptographic signature/trusted provenance, GitHub Release, and independent verification remain |
| RELEASE-005 | Security and dependency scanning | in_progress | Local release gate records zero medium-or-higher Bandit findings, zero unresolved findings in 157 secret-scanned code/document files, and zero known advisories among 23 exact locked runtime dependencies at check time; security and CodeQL workflows exist, but no remote result is claimed until GitHub runs them |
