...
| RELEASE-004 | Signed release, checksums and SBOM | in_progress | Local candidate emits a validated CycloneDX 1.6 SBOM, portable per-asset checksum inventory, deterministic release bundle, and detached bundle SHA-256. A cryptographic signature/trusted provenance, GitHub Release, and independent verification remain |
...
| RELEASE-007 | Claim-language audit | verified | A deterministic policy scans governed source/document surfaces, rejects false project/hosted URLs and stale publication language, reconciles quantitative statements against versioned JSON evidence, requires explicit scope boundaries, records the absent local Git remote and package project URLs, and refreshes the dated no-public-repository assertion against the official GitHub API. Offline release builds re-run the audit against the committed public-state snapshot without turning it into external validation |
